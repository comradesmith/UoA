...
class Polynomial:
	...
	def scale(self, factor):
		new_coefficients = [old_coefficient * factor for old_coefficient in self.coefficients]
	...
	def evaluate(self, x):
		...
		for i in range(len(powers)):
			sum += self.coefficients[i] * (x ** powers[i])
		return sum
	...
	def get_powers(self):
		return [position - 1 for position in range(len(self.coefficients),0,-1)]
...
class Course:
	...
	def class_list(self):
		return ' '.join([student.get_name() for student in self.students])
